...
const PromptsUsage = () => {
  ...
  const [bookings, setBookings] = React.useState<BookingCounts>({ bookingTotalCount: 0 });
  const [isLoading, setIsLoading] = React.useState(true);

  const fetchtotalBookingCounts = async () => {
    try {
      setIsLoading(true);
      const res = await axios.get(`${baseURL}/ambulance/ambulance_bookings_count`);
      setBookings(res.data?.jsonData?.total_booking_count || {});
      // console.log("API Response of Booking: ", res.data?.jsonData?.total_booking_count);
    } catch (error) {
      console.error("Error fetching booking counts: ", error);
      setBookings({ bookingTotalCount: 0 });
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <Card className="card-h-100">
      <CardBody>
        <div className="d-flex justify-content-between align-items-start">
          <div>
            <h5 className="text-uppercase mb-3">Booking ({bookings.bookingTotalCount})</h5>
          </div>
          <div>
