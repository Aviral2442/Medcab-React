...
const vehicleValidationSchema = Yup.object().shape({
  ...
  vehicle_category_type: Yup.string().required(
    "Vehicle category type is required"
  ),
  vehicle_category_type_service_id: Yup.string().required(
    "Vehicle category type service ID is required"
  ),
  ...
  if (/^\d+$/.test(s)) {
    try {
      ...
      if (!isNaN(dGeneric.getTime()))
        return dGeneric.toISOString().slice(0, 10);
    } catch {}
  }
...
const AddVehicle: React.FC = () => {
  ...
  const fetchVehicle = async () => {
    ...
    try {
      ...
      setInitialValues({
        ...
        vehicle_details_fitness_exp_date: toInputDate(
          vehicle?.vehicle_details_fitness_exp_date
        ),
        vehicle_details_insurance_img: null,
        vehicle_details_insurance_exp_date: toInputDate(
          vehicle?.vehicle_details_insurance_exp_date
        ),
        vehicle_details_insurance_holder_name: vehicle?.vehicle_details_insurance_holder_name || "",
        vehicle_details_pollution_img: null,
        vehicle_details_pollution_exp_date: toInputDate(
          vehicle?.vehicle_details_pollution_exp_date
        ),
      });
      ...
      if (vehicle?.vehicle_details_fitness_certi_img) {
        setPreviewFitness(
          `${baseURL}/${vehicle.vehicle_details_fitness_certi_img}`
        );
      }
      if (vehicle?.vehicle_details_insurance_img) {
        setPreviewInsurance(
          `${baseURL}/${vehicle.vehicle_details_insurance_img}`
        );
      }
      if (vehicle?.vehicle_details_pollution_img) {
        setPreviewPollution(
          `${baseURL}/${vehicle.vehicle_details_pollution_img}`
        );
      }

      // Automatically fetch added by details if vehicle_added_type and vehicle_added_by exist
      if (vehicle?.vehicle_added_type) {
        await fetchAddedByList(vehicle.vehicle_added_type.toString());
      }
    } catch (err: any) {
  ...
  const fetchAddedByList = async (addedType: string) => {
    if (!addedType) return;
    ...
    try {
      ...
      if (addedType === "0") {
        url = `${baseURL}/vehicle/get_vehicle_added_by_driver_details`;
      } else if (addedType === "1") {
        url = `${baseURL}/vehicle/get_vehicle_added_by_partner_details`;
      }
  ...
  const handleSubmit = async (values: typeof initialValues) => {
    ...
    try {
      ...
      formData.append(
        "vehicle_category_type_service_id",
        values.vehicle_category_type_service_id
      );
      formData.append("v_vehicle_name", values.v_vehicle_name);
      formData.append("v_vehicle_name_id", values.v_vehicle_name_id);
      if (values.vehicle_front_image)
        formData.append("vehicle_front_image", values.vehicle_front_image);
      if (values.vehicle_back_image)
        formData.append("vehicle_back_image", values.vehicle_back_image);
      if (values.vehicle_rc_image)
        formData.append("vehicle_rc_image", values.vehicle_rc_image);
      formData.append("vehicle_rc_number", values.vehicle_rc_number);
      // send backend-friendly datetime format
      formData.append(
        "vehicle_exp_date",
        toBackendDateTime(values.vehicle_exp_date)
      );
      if (values.vehicle_verify_date)
        formData.append(
          "vehicle_verify_date",
          toBackendDateTime(values.vehicle_verify_date)
        );
      if (values.verify_type)
        formData.append("verify_type", values.verify_type);

      // Details
      if (values.vehicle_details_fitness_certi_img)
        formData.append(
          "vehicle_details_fitness_certi_img",
          values.vehicle_details_fitness_certi_img
        );
      formData.append(
        "vehicle_details_fitness_exp_date",
        toBackendDateTime(values.vehicle_details_fitness_exp_date)
      );
      if (values.vehicle_details_insurance_img)
        formData.append(
          "vehicle_details_insurance_img",
          values.vehicle_details_insurance_img
        );
      formData.append(
        "vehicle_details_insurance_exp_date",
        toBackendDateTime(values.vehicle_details_insurance_exp_date)
      );
      formData.append(
        "vehicle_details_insurance_holder_name",
        values.vehicle_details_insurance_holder_name
      );
      if (values.vehicle_details_pollution_img)
        formData.append(
          "vehicle_details_pollution_img",
          values.vehicle_details_pollution_img
        );
      formData.append(
        "vehicle_details_pollution_exp_date",
        toBackendDateTime(values.vehicle_details_pollution_exp_date)
      );

      // Append details added type / by
      if (values.vehicle_details_added_type)
        formData.append(
          "vehicle_details_added_type",
          values.vehicle_details_added_type
        );
      if (values.vehicle_details_added_by)
        formData.append(
          "vehicle_details_added_by",
          values.vehicle_details_added_by
        );

      let response;
      if (isEditMode) {
        response = await axios.put(
          `${baseURL}/vehicle/update_vehicle/${id}`,
          formData,
          {
            headers: {
            ...
            <Row className="g-3">
                      ...
                      <Col md={4}>
                        <Form.Group>
                          <Form.Label className="fs-6 fw-semibold">
                            Added Type <span className="text-danger">*</span>
                          </Form.Label>
                          <Form.Select
                            ...
                            onChange={(e) => {
                              ...
                              setFieldValue("vehicle_added_by", "");
                              setAddedByOptions([]);

                              // fetch driver or partner list
                              fetchAddedByList(e.target.value);
                            }}
                          >
                            <option value="">Select</option>
                            <option value="0">Self (Driver)</option>
                            <option value="1">Partner</option>
                          </Form.Select>

                          <Form.Control.Feedback type="invalid">
                            {errors.vehicle_added_type}
                      ...
                      <Col md={4}>
                        <Form.Group>
                          ...
                          <Form.Select
                            ...
                            onBlur={handleBlur}
                            isInvalid={
                              touched.vehicle_category_type &&
                              !!errors.vehicle_category_type
                            }
                          >
                      ...
                      <Col md={6}>
                        <Form.Group>
                          ...
                          <Form.Control
                            ...
                            onBlur={handleBlur}
                            isInvalid={
                              touched.vehicle_category_type_service_id &&
                              !!errors.vehicle_category_type_service_id
                            }
                      ...
                      <Col md={6}>
                        <Form.Group>
                          ...
                          <Form.Control
                            ...
                            onBlur={handleBlur}
                            isInvalid={
                              touched.v_vehicle_name && !!errors.v_vehicle_name
                            }
                            placeholder="Enter vehicle name"
                          />
                          <Form.Control.Feedback type="invalid">
                            {errors.v_vehicle_name}
                          </Form.Control.Feedback>
                        </Form.Group>
                      </Col>
                      ...
                      <Col md={6}>
                        <Form.Group>
                          ...
                          <Form.Control
                            ...
                            onBlur={handleBlur}
                            isInvalid={
                              touched.v_vehicle_name_id &&
                              !!errors.v_vehicle_name_id
                            }
                    ...
                    <Row className="g-3">
                      <Col md={4}>
                        <Form.Group>
                          ...
                          <Form.Control
                            ...
                            onBlur={handleBlur}
                            isInvalid={
                              touched.vehicle_front_image &&
                              !!errors.vehicle_front_image
                            }
                          />
                          {touched.vehicle_front_image &&
                            (errors as any).vehicle_front_image && (
                      ...
                      <Col md={4}>
                        <Form.Group>
                          ...
                          <Form.Control
                            ...
                            onBlur={handleBlur}
                            isInvalid={
                              touched.vehicle_back_image &&
                              !!errors.vehicle_back_image
                            }
                          />
                          {touched.vehicle_back_image &&
                            (errors as any).vehicle_back_image && (
                              <div className="text-danger small mt-1">
                                {(errors as any).vehicle_back_image}
                              </div>
                            )}
                      ...
                      <Col md={4}>
                        <Form.Group>
                          ...
                          <Form.Control
                            ...
                            onBlur={handleBlur}
                            isInvalid={
                              touched.vehicle_rc_image &&
                              !!errors.vehicle_rc_image
                            }
                          />
                          {touched.vehicle_rc_image &&
                            (errors as any).vehicle_rc_image && (
                              <div className="text-danger small mt-1">
                                {(errors as any).vehicle_rc_image}
                              </div>
                            )}
                      ...
                      <Col md={4}>
                        <Form.Group>
                          ...
                          <Form.Control
                            ...
                            onBlur={handleBlur}
                            isInvalid={
                              touched.vehicle_rc_number &&
                              !!errors.vehicle_rc_number
                            }
                      ...
                      <Col md={4}>
                        <Form.Group>
                          ...
                          <Form.Control
                            type="date"
                            name="vehicle_exp_date"
                            value={values.vehicle_exp_date}
                            onChange={handleChange}
                            onBlur={handleBlur}
                            isInvalid={
                              touched.vehicle_exp_date &&
                              !!errors.vehicle_exp_date
                            }
                          />
                          <Form.Control.Feedback type="invalid">
                            {errors.vehicle_exp_date}
                      ...
                      <Col md={4}>
                        <Form.Group>
                          ...
                          <Form.Select
                            name="verify_type"
                            value={values.verify_type}
                            onChange={handleChange}
                            onBlur={handleBlur}
                            isInvalid={
                              touched.verify_type && !!errors.verify_type
                            }
                          >
                    ...
                    <Row className="g-3">
                      <Col md={4}>
                        <Form.Group>
                          ...
                          <Form.Select
                            ...
                            onChange={(e) => {
                              handleChange(e);
                              // Clear details added by when type changes
                              setFieldValue("vehicle_details_added_by", "");
                            }}
                            onBlur={handleBlur}
                            isInvalid={
                              touched.vehicle_details_added_type &&
                              !!errors.vehicle_details_added_type
                            }
                          >
                            <option value="">Select</option>
                            <option value="0">Self (Driver)</option>
                      ...
                      <Col md={4}>
                        <Form.Group>
                          <Form.Label className="fs-6 fw-semibold">
                            Details Added By{" "}
                            {values.vehicle_details_added_type === "0"
                              ? "(Driver ID)"
                              : values.vehicle_details_added_type === "1"
                              ? "(Partner ID)"
                              : ""}
                          </Form.Label>
                          <Form.Control
                            ...
                            onBlur={handleBlur}
                            isInvalid={
                              touched.vehicle_details_added_by &&
                              !!errors.vehicle_details_added_by
                            }
                            placeholder={`Enter ${
                              values.vehicle_details_added_type === "0"
                                ? "Driver"
                            ...
                            disabled={!values.vehicle_details_added_type}
                          />
                          <Form.Control.Feedback type="invalid">
                            {errors.vehicle_details_added_by}
                          </Form.Control.Feedback>
                      ...
                      <Col md={4}>
                        <Form.Group>
                          ...
                          <Form.Control
                            ...
                            onBlur={handleBlur}
                            isInvalid={
                              touched.vehicle_details_fitness_certi_img &&
                              !!errors.vehicle_details_fitness_certi_img
                            }
                          />
                          {touched.vehicle_details_fitness_certi_img &&
                            (errors as any)
                      ...
                      <Col md={4}>
                        <Form.Group>
                          ...
                          <Form.Control
                            type="date"
                            name="vehicle_details_fitness_exp_date"
                            value={values.vehicle_details_fitness_exp_date}
                            onChange={handleChange}
                            onBlur={handleBlur}
                          />
                        </Form.Group>
                      ...
                      <Col md={4}>
                        <Form.Group>
                          ...
                          <Form.Control
                            ...
                            onBlur={handleBlur}
                            isInvalid={
                              touched.vehicle_details_insurance_img &&
                              !!errors.vehicle_details_insurance_img
                            }
                          />
                          {touched.vehicle_details_insurance_img &&
                            (errors as any).vehicle_details_insurance_img && (
                              <div className="text-danger small mt-1">
                                {(errors as any).vehicle_details_insurance_img}
                              </div>
                            )}
                      ...
                      <Col md={4}>
                        <Form.Group>
                          ...
                          <Form.Control
                            type="date"
                            name="vehicle_details_insurance_exp_date"
                            value={values.vehicle_details_insurance_exp_date}
                            onChange={handleChange}
                            onBlur={handleBlur}
                          />
                        </Form.Group>
                      ...
                      <Col md={4}>
                        <Form.Group>
                          ...
                          <Form.Control
                            type="text"
                            name="vehicle_details_insurance_holder_name"
                            value={values.vehicle_details_insurance_holder_name}
                            onChange={handleChange}
                            onBlur={handleBlur}
                      ...
                      <Col md={4}>
                        <Form.Group>
                          ...
                          <Form.Control
                            ...
                            onBlur={handleBlur}
                            isInvalid={
                              touched.vehicle_details_pollution_img &&
                              !!errors.vehicle_details_pollution_img
                            }
                          />
                          {touched.vehicle_details_pollution_img &&
                            (errors as any).vehicle_details_pollution_img && (
                              <div className="text-danger small mt-1">
                                {(errors as any).vehicle_details_pollution_img}
                              </div>
                            )}
              ...
              <Col lg={12}>
                <div className="d-flex gap-2 justify-content-end">
                  <button
                    className="px-3 border-0 rounded text-black"
                    onClick={() => navigate("/ambulance/vehicle")}
                    disabled={submitting}
                  >
                    Cancel
                  </button>
