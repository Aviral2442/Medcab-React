...
const CategoryValidationSchema = Yup.object().shape({
    ...
    ambulance_category_service_type: Yup.string().required('Service Type is required'),
...
const AddCategory: React.FC<AddCategoryProps> = ({ mode, data, onCancel, onDataChanged }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await CategoryValidationSchema.validate(formValues, { abortEarly: false });

            const formData = new FormData();
            formData.append('ambulance_category_type', formValues.ambulance_category_type);
            formData.append('ambulance_category_service_type', formValues.ambulance_category_service_type);
            formData.append('ambulance_category_state_id', formValues.ambulance_category_state_id);
            formData.append('ambulance_category_name', formValues.ambulance_category_name);
            formData.append('ambulance_catagory_desc', formValues.ambulance_catagory_desc);
            if (formValues.iconPath) {
                formData.append('iconPath', formValues.iconPath);
            }

            const endpoint = mode === 'add' ? '/ambulance/add_ambulance_category' : '/ambulance/edit_ambulance_category';
