    ...
    return (
        <Container fluid>
            <PageTitle
                ...
                subtitle="Visualize chronological events, project milestones, or user actions in a streamlined, scrollable view."
                badge={{
                        ...
                        <div className="timeline">
                            {timelineItems1.map((item, idx) => (
                                <div key={idx} className="timeline-item d-flex align-items-stretch">
                                    ...
                                    <div
                                        className={`timeline-content ps-3 ${idx != timelineItems1.length - 1 ? 'pb-4' : ''}`}>
                        ...
                        <div className="timeline timeline-icon-based">
                            {timelineItems2.map((item, idx) => (
                                <div key={idx} className="timeline-item d-flex align-items-stretch">
                                    ...
                                    <div
                                        className={`timeline-content ps-3 ${idx != timelineItems2.length - 1 ? 'pb-4' : ''}`}>
                                        <h5 className="mb-1">{item.title}</h5>
                                        <p className="mb-1 text-muted">{item.description}</p>
                                        <span className="text-primary fw-semibold">By {item.author}</span>
                                    </div>
                        ...
                        <div className="timeline timeline-icon-bordered">
                            {timelineItems3.map((item, idx) => (
                                <div key={idx} className="timeline-item d-flex align-items-stretch">
                                    ...
                                    <div
                                        className={`timeline-content ps-3 ${idx != timelineItems3.length - 1 ? 'pb-4' : ''}`}>
                                        <h5 className="mb-1">{item.title}</h5>
                                        <p className="mb-1 text-muted">{item.description}</p>
                                        <span className="text-primary fw-semibold">By {item.author}</span>
                                    </div>
                        ...
                        <div className="timeline timeline-users">
                            {timelineItems4.map((item, idx) => (
                                <div key={idx} className="timeline-item d-flex align-items-stretch">
                                    {item.image && (
                                        <div className="timeline-dot">
                                            <img src={item.image} width={30} height={30} alt="avatar-1"
                                                 className="img-fluid rounded-circle"/>
                                        </div>
                                    )}
                                    <div
                                        className={`timeline-content ps-3 ${idx != timelineItems4.length - 1 ? 'pb-4' : ''}`}>
                                        <h5 className="mb-1">{item.title}</h5>
                                        <p className="mb-1 text-muted">{item.description}</p>
                                        <span className="text-primary fw-semibold">By {item.author}</span>
                                    </div>
