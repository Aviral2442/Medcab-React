  ...
  const fetchStatsData = async () => {
    try {
      const res = await axios.get(
        `${basePath}/dashboard/get_vendor_today_yesterday_counts`
      );
        ...
        <Row className="align-items-center">
          <Col xl={3} md={6}>
            <div className="text-center">
              ...
              <h2 className="fw-bold mb-0">
                <span>
                  <CountUp
                    end={statsData.total_amount_last_30_days}
                    duration={2}
                    enableScrollSpy
                    scrollSpyOnce
                  />
          ...
          <Col xl={3} md={6} className="order-xl-last">
            <div className="text-center">
              ...
              <h2 className="fw-bold mb-0">
                {formatIndianNumber(statsData.overall_wallet_amount)}{" "}
              </h2>
              <p className="text-muted">Total Amount In Wallet</p>
              <p className="mb-0 mt-4">
                <LuClock /> Last Transaction On:{" "}
                {convertDate(statsData.last_transaction_unix_time)}
              </p>
            </div>
          </Col>
