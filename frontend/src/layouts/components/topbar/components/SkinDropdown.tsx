...
import type {LayoutSkinType} from '@/types/layout'
...
const SkinDropdown = () => {
    const {skin, changeSkin} = useLayoutContext()
    ...
    const currentSkin = useMemo(() => {
        if (!mounted || !skin) return {name: 'Loading...', img: shadcn}
        return skinOptions.find((s) => s.name === skin) ?? skinOptions[0]
    }, [mounted, skin])

    const handleSkinChange = (option: SkinOptionType) => {
        ...
        <div className="topbar-item me-2">
            <Dropdown align="end">
                <DropdownToggle as={'button'} className="topbar-link fw-semibold drop-arrow-none">
                    <img src={currentSkin.img} alt="skin-image" className="w-100 rounded me-2" height={18} width={18}
                ...
                <DropdownMenu className="dropdown-menu-lg p-1">
                    <SimpleBar className="h-100" style={{maxHeight: '250px'}}>
                        <Row className="g-0">
                            {chunkArray(skinOptions, 7).map((chunk, idx) => (
                                <Col md={6} key={idx}>
                                    {chunk.map((item, index) => (
                                        <button
                                            key={index}
                                            className={clsx('dropdown-item position-relative', item.name === currentSkin.name && 'drop-custom-active')}
