...
const BookingDetailsApiData = () => {
  ...
  const updateScheduleTime = async (bookingId: string, scheduleTime: string) => {
    try {
  ...
  const searchConsumers = async (query: string) => {
    try {
      const response = await axios.get(
        `${baseURL}/ambulance/get_ambulance_consumer_mobile?search=${encodeURIComponent(query)}`
      );
      return {
        success: true,
  ...
  const updateConsumerDetails = async (
    ...
    try {
      ...
      return {
        success: false,
        message: error.response?.data?.message || "Failed to update consumer details"
      };
    }
  };
  ...
  const searchVehicles = async (query: string) => {
    try {
      const response = await axios.get(
        `${baseURL}/ambulance/get_vehicle_and_assign_data?search=${encodeURIComponent(query)}`
      );
      return {
        success: true,
