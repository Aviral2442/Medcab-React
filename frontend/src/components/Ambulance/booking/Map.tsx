import { Card, CardBody, CardHeader, CardTitle, Col, Container } from "react-bootstrap";
...
const Map = ({ data }: MapProps) => {
  ...
  const centerLat = (parseFloat(data?.booking_pick_lat) + parseFloat(data?.booking_drop_lat)) / 2;
  const centerLong = (parseFloat(data?.booking_pick_long) + parseFloat(data?.booking_drop_long)) / 2;
  ...
  if (data?.booking_polyline) {
    try {
      ...
      polylineCoordinates = decoded.map(coord => [coord[0], coord[1]] as LatLngExpression);
    } catch (error) {
