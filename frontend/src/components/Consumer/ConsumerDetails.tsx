...
interface FieldDef {
  label: string;
  name: string;
  type?: "text" | "number" | "tel" | "email" | "date" | "datetime-local" | "select";
  options?: { value: string | number; label: string }[];
  rows?: number;
}

const Field: React.FC<{ label: string; value: any; type?: string; options?: any[]; rows?: number }> = ({
  label,
  value,
  type = "text",
  options = [],
  ...
  const format = (val: any) => {
    ...
    if ((type === "date" || type === "datetime-local") && s) {
      try {
        const date = /^\d+$/.test(s) ? new Date(parseInt(s, 10) * 1000) : new Date(s);
    ...
    if (type === "email" || type === "tel" || type === "text" || type === "number") return s;
...
const consumerFields: FieldDef[] = [
  ...
  { label: "Registered Date", name: "consumer_registred_date", type: "datetime-local" },
              ...
              <Col md={3} key={f.name}>
                {f.name === "consumer_status" ? (
                  <div className="mb-3">
                    ...
                    <div className="d-flex align-items-center">
                      <div className="d-flex align-items-center flex-grow-1 border rounded p-2">
                        <div className="flex-grow-1">{(data?.consumer_status)}</div>
                      </div>
                    </div>
                  </div>
                ) : f.name === "consumer_registred_date" ? (
                  <Field label={f.label} value={formatDateShort(data?.[f.name])} type={f.type} />
                ) : (
