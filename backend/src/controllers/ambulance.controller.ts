...
// COUNT DASHBOARD AMBULANCE BOOKING COUNT CONTROLLER
export const countDashboardAmbulanceBookingController = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const result = await getAmbulanceCategoryListService(filters);
        res.status(200).json(result);
...
// PARTNER DASHBOARD COUNT CONTROLLER
export const partnerDashboardCountController = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const filters = {
            date: req.query.date as string,
            status: req.query.status as string,
...
export const getAmbulanceConsumerMobileController = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { search } = req.query;

        const result = await getAmbulanceConsumerMobileService(
...
// CONTROLLER TO GET AMBULANCE VEHICLE AND ASSIGN DATA (SEARCHABLE)
export const getAmbulanceVehicleAndAssignDataController = async (
    req: Request,
    res: Response,
    next: NextFunction
) => {
    try {
        ...
        const result = await getAmbulanceVehicleAndAssignDataService(search);
...
export const assignDriverController = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const { driverId, vehicleId, adminId } = req.body;

        if (!bookingId || !driverId || !vehicleId || !adminId) {
            return res.status(400).json({
                status: 400,
                message: "Booking ID, Driver ID, Vehicle ID and Admin ID are required",
            });
        }
...
// CONTROLLER TO GET CANCEL REASONS
export const cancelReasonController = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const result = await cancelReasonService();
        res.status(200).json(result);
    } catch (error) {
        next(error);
    }
...
// CONTROLLER TO CANCEL AMBULANCE BOOKING
export const cancelAmbulanceBookingController = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        console.log(bookingId, cancelReasonId, cancelReason, cancelUserType, adminId);
...
// CONTROLLER TO VERIFY OTP FOR AMBULANCE BOOKING
...
// CONTROLLER TO CANCEL DRIVER FROM AMBULANCE BOOKING
export const cancelDriverFromAmbulanceBookingController = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const bookingId = parseInt(req.params.id);
...
export const ambulanceBookingInvoiceController = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const bookingId = parseInt(req.params.bookingId);
        const result = await ambulanceBookingInvoiceSerive(bookingId);
        res.status(200).json(result);
    } catch (error) {
        next(error);
    }
...
// CONTROLLER TO GET AMBULANCE ACCEPT HISTORY LIST
export const ambulanceAcceptHistoryListController = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const filters = {
            page: req.query.page ? parseInt(req.query.page as string) : undefined,
            limit: req.query.limit ? parseInt(req.query.limit as string) : undefined,
        ...
        const bookingId = parseInt(req.params.bookingId);
        const result = await ambulanceBookingDriverAcceptHistoryService(bookingId, filters);
        res.status(200).json(result);
    }
    catch (error) {
        next(error);
...
// CONTROLLER TO GET AMBULANCE REJECT HISTORY LIST
export const ambulanceRejectHistoryListController = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const filters = {
            page: req.query.page ? parseInt(req.query.page as string) : undefined,
            limit: req.query.limit ? parseInt(req.query.limit as string) : undefined,
        ...
        const bookingId = parseInt(req.params.bookingId);
        const result = await ambulanceBookingDriverRejectHistoryService(bookingId, filters);
        res.status(200).json(result);
    } catch (error) {
        next(error);
    }
